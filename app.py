...
DB_PATH = OUTPUT_DIR / "studio2_definitivo_risposte.sqlite"
CSV_RESPONSES = OUTPUT_DIR / "studio2_definitivo_risposte.csv"
EXCEL_RESPONSES = OUTPUT_DIR / "studio2_definitivo_risposte.xlsx"

MAX_P = 90
TEAM_SIZE = 9
N_TEAMS = MAX_P // TEAM_SIZE
BASE_SEED = 2026
RESERVATION_HOURS = 0.5
...
PROFILI_PROFESSIONALI = [
    "Manager / dirigente",
    "Analista",
    "Impiegato / addetto operativo",
    "Professionista tecnico / specialista",
    "Ricercatore / accademico",
    "Altro",
]

# Il profilo professionale non determina automaticamente il potere nel team.
# Nel disegno sperimentale la leadership dipende esclusivamente dalla posizione.
TEAM_ROLE_INFLUENCE = {
    "Team Leader": 1.00,
    "Componente del team": 0.55,
}
...
def parse_probability_input(value: Any) -> float | None:
    """Converte una stima esplicita 0-100, accettando punto o virgola."""
...
# Raccolta definitiva v1. I fogli originari Risposte, Prenotazioni e Log
# restano intatti e costituiscono l'archivio della fase pilota (30 casi).
# La nuova raccolta riparte da participant_index 0 con lo stesso schema dati.
RISPOSTE_SHEET = "Risposte_Definitivo"
PRENOTAZIONI_SHEET = "Prenotazioni_Definitivo"
LOG_SHEET = "Log_Definitivo"
...
def coerce_numeric_response_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in numeric_columns:
        if column in result.columns:
            # I fogli con impostazioni locali italiane restituiscono spesso i
            # decimali come testo con la virgola. La normalizzazione evita che
            # P, G, H e F vengano trasformati in valori mancanti nel back office.
            normalized = result[column].astype(str).str.strip().str.replace(
                ",", ".", regex=False
            )
            result[column] = pd.to_numeric(normalized, errors="coerce")
...
def _active_and_completed_indices() -> tuple[set[int], set[int]]:
    ...
    for record in reservations:
        ...
        try:
            ...
            participant_index = int(float(record.get("participant_index")))
        except (TypeError, ValueError):
            continue
...
def reserve_slot() -> tuple[str, int]:
    """Riserva uno slot persistente riducendo al minimo le letture di Google Sheets.

    Strategia:
    - una lettura iniziale per determinare gli slot completati/attivi;
    - una sola append sul foglio Prenotazioni;
    - una breve attesa;
    - una sola rilettura di verifica anti-collisione;
    - al massimo un secondo tentativo in caso di vera collisione.
    """
    ...
    for attempt in range(2):
        ...
        own_row_number: int | None = None

        for row_number, record in enumerate(records, start=2):
            try:
                same_index = int(float(record.get("participant_index", ""))) == int(available)
            except (TypeError, ValueError):
                same_index = False
            ...
            record_token = str(record.get("session_token", "")).strip()
            record_status = str(record.get("status", "")).strip().lower()
            record_created_raw = str(record.get("created_at", "")).strip()

            if record_token == token:
                own_row_number = row_number

            # Devono competere solo le prenotazioni ancora valide.
            # Le righe "attiva" più vecchie di RESERVATION_HOURS sono scadute
            # e non devono impedire il riutilizzo dello slot.
            if record_status == "attiva":
                try:
                    record_created = datetime.fromisoformat(record_created_raw)
                except (TypeError, ValueError):
                    continue

                if record_created >= datetime.now() - timedelta(hours=RESERVATION_HOURS):
                    contenders.append(
                        (
                            record_created_raw,
                            record_token,
        ...
        if own_row_number is None:
            if attempt == 0:
                time.sleep(1.2)
                continue
            raise RuntimeError(
                "Google Sheets sta rispondendo lentamente. "
                "Attendi circa un minuto e riprova."
            )
    ...
    raise RuntimeError(
        "Non è stato possibile completare la prenotazione dello slot. "
        "Attendi pochi secondi e riprova."
    )
...
def ruolo_sperimentale(position: int) -> str:
    """Ruolo assegnato dal disegno, distinto dal profilo professionale."""
    return "Team Leader" if int(position) == 0 else "Componente del team"
...
def prepare_team(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "ruolo" not in team.columns:
        team["ruolo"] = "Profilo non disponibile"
    team["team_role"] = team["posizione_team"].apply(ruolo_sperimentale)
    team["role_influence"] = team["team_role"].map(TEAM_ROLE_INFLUENCE).fillna(0.55)
    return team
...
def build_network(team: pd.DataFrame, t: str) -> tuple[nx.Graph, list[str], int, float]:
    ...
    leader_candidates = team.index[
        pd.to_numeric(team["posizione_team"], errors="coerce") == 0
    ].tolist()
    leader_idx = int(leader_candidates[0]) if leader_candidates else 0
    for i, code in enumerate(codes):
        graph.add_node(
            code,
            role=str(team.iloc[i]["team_role"]),
            professional_profile=str(team.iloc[i]["ruolo"]),
        )
    for i, code in enumerate(codes):
        if i != leader_idx:
...
st.progress(
    current_completed / MAX_P,
    text=f"Risposte completate: {current_completed}/{MAX_P}",
)
...
if "profile" not in st.session_state:
    ...
    experimental_role = ruolo_sperimentale(int(assignment["posizione_team"]))
    st.info(
        f"Dominio assegnato: **{DOMINI[dominio]}**  \n"
        f"Ruolo nel team sperimentale: **{experimental_role}**"
    )
    with st.form("profile_form"):
        professional_profile = st.selectbox(
            "Quale profilo descrive meglio la sua attuale posizione professionale?",
            ["— seleziona —"] + PROFILI_PROFESSIONALI,
        )
    ...
    if start:
        if (
            professional_profile == "— seleziona —"
            or experience == "— seleziona —"
            or specialist_area == "— seleziona —"
        ):
            ...
            # La colonna storica "ruolo" conserva ora il profilo professionale.
            # Il ruolo sperimentale si ricava senza nuove colonne da posizione_team.
            st.session_state["ruolo"] = professional_profile
            st.rerun()
    st.stop()
...
with st.expander("Istruzioni operative", expanded=True):
    st.markdown(
        ...
        4. Formula una nuova valutazione finale senza partire dalla scelta
           conferma/modifica.
...
Dopo aver letto l'analisi, considera nuovamente tutte le informazioni
disponibili e formula la valutazione che ritieni corretta in questo momento.

Inserisci esplicitamente un valore da 0 a 100. Il valore può anche coincidere
con la valutazione iniziale, ma deve essere inserito nuovamente.
""")

    st.markdown("### Valutazione finale autonoma")
    st.text_input(
        "Considerando il briefing e l'analisi appena letta, qual è ora la tua valutazione finale? (0–100)",
        value="",
        key=f"{t}_post_final_input",
        placeholder="Inserisci un numero da 0 a 100",
    )

    st.selectbox(
        "Le informazioni disponibili sono sufficienti per formulare la valutazione finale?",
        [
            "— seleziona —",
            "Le informazioni sono sufficienti",
            "Le informazioni non sono sufficienti",
        ],
        key=f"{t}_suffic_base",
    )
    st.selectbox(
    ...
    if not st.session_state.get(f"{t}_completed", False):
        if st.button(f"Completa la condizione {t}", key=f"{t}_complete", type="primary"):
            temp_errors = []
            post = parse_probability_input(
                st.session_state.get(f"{t}_post_final_input", "")
            )
            if post is None:
                temp_errors.append("valutazione finale numerica da 0 a 100")
            for key, label in [
                (f"{t}_suffic_base", "sufficienza informativa"),
                (f"{t}_motivo", "motivazione"),
            ]:
            ...
            if temp_errors:
                st.warning(
                    "Completa correttamente i seguenti campi: "
                    + "; ".join(temp_errors)
                    + "."
                )
            else:
                # Le categorie storiche vengono derivate dopo la nuova stima.
                # I nomi delle colonne e i valori trasmessi a Google Sheets
                # restano quindi identici alla fase pilota.
                unchanged = abs(float(post) - pre_saved) < 1e-9
                choice = (
                    "Confermo la mia valutazione iniziale"
                    if unchanged
                    else "Modifico la valutazione iniziale"
                )
                suffic_base = st.session_state[f"{t}_suffic_base"]
                suffic = (
                    f"{suffic_base} — confermo la valutazione iniziale"
                    if unchanged
                    else f"{suffic_base} — modifico la valutazione iniziale"
                )
                st.session_state[f"{t}_post_slider"] = float(post)
                st.session_state[f"{t}_post_choice"] = choice
                st.session_state[f"{t}_post_ord"] = ordinal_from_probability(float(post))
                st.session_state[f"{t}_suffic"] = suffic
